...
class Ezoic_AdsTxtManager extends Ezoic_Feature {
	...
	function ezoic_adstxtmanager_display_notice() {
		...
		if (!is_int($adstxtmanager_id)) {
		...
		} else if (in_array($pagenow, array('options-general.php')) && $_GET['page'] == EZOIC__PLUGIN_SLUG && isset($_GET['tab']) && $_GET['tab'] == 'adstxtmanager_settings') {

			if ((isset($_GET['verify']) && $_GET['verify']) || !$adstxtmanager_status || (isset($adstxtmanager_status['status']) && $adstxtmanager_status['status'] == false && $adstxtmanager_id > 0)) {
			...
			if ($adstxtmanager_id > 0 && isset($adstxtmanager_status['status'])) {
	...
	public static function ezoic_should_show_adstxtmanager_setting() {
		...
		if (
				isset( $options['is_integrated'] ) == false
				|| $options['is_integrated'] == false
				|| Ezoic_Integration_Admin::is_cloud_integrated()
			/*|| ( isset( $ezoic_options['disable_wp_integration'] )
