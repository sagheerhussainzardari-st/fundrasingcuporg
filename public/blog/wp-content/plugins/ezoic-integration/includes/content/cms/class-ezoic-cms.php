...
class Ezoic_CMS extends Ezoic_Feature {
	...
	public function register_sync_hooks( $loader ) {
		...
		$loader->add_action( 'edit_term', $this->cms_sync, 'term_updated' );
		$loader->add_action( 'pre_delete_term', $this->cms_sync, 'term_deleted' );
		$loader->add_action( 'customize_save_after', $this->cms_sync, 'logo_update' );

		// $loader->add_action( 'draft_to_draft', $this->cms_sync, 'post_updated' );
		// $loader->add_action( 'publish_to_publish', $this->cms_sync, 'post_updated' );
		// $loader->add_action( 'draft_to_publish', $this->cms_sync, 'post_updated' );
		// $loader->add_action( 'publish_to_draft', $this->cms_sync, 'post_updated' );
		// $loader->add_action( 'auto-draft_to_draft', $this->cms_sync, 'post_added' );
		// $loader->add_action( 'auto-draft_to_publish', $this->cms_sync, 'post_added' );
		$loader->add_action( 'wp_after_insert_post', $this->cms_sync, 'post_saved', 10, 3 );
		$loader->add_action( 'publish_to_trash', $this->cms_sync, 'post_deleted' ); // for page too?
		$loader->add_action( 'draft_to_trash', $this->cms_sync, 'post_deleted' );
		$loader->add_action( 'auto-draft_to_trash', $this->cms_sync, 'post_deleted');
