...
class Ezoic_Content_Request {
	...
	public static function send_request( $url_path, $request ) {
		...
		$request["headers"]["X-Ezoic-CMS-Auth"] = "Bearer " . $token;
	...
	public static function send_sync_options( $options ) {
		...
		return self::send_request( '/options', $request );
	}
	...
	public static function send_export_status( $status, $module ) {
		...
		return self::send_request( '/import/wp/status', $request );
	}

	public static function send_export_alert( $success, $message, $module ) {
		// Send alert to CMS
		$payload = array(
		...
		return self::send_request( '/import/wp/alert', $request );
	}

	public static function send_file_upload( $file_path, $filename, $auth_token, $cms_id ) {
		if ( !$auth_token ) {
		...
		$options_set = curl_setopt_array( $curl, array(
			CURLOPT_URL => "https://content-backend.ezoic.com/api/v1/tenants/$cms_id/upload/wp",
