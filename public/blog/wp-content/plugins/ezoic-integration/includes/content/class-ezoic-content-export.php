...
abstract class Ezoic_Content_Export {
	...
	abstract protected function get_export_filenames();
	...
	protected function schedule_export_cron() {
	...
	public function attempt_archive_upload( $tenant ) {
		...
		return Ezoic_Content_Request::send_file_upload( $export_archive, $filename,
			$token, $tenant );
	}

	protected function send_alert( $success, $failureInfo ) {
		delete_transient( $this->get_transient_name() );
		...
		return Ezoic_Content_Request::send_export_alert( $success, $message, $this->get_module_name() );
	}
	...
	public function verify_export_files() {
		...
		$filesizes = $file->verify_files( $this->get_export_filenames() );
